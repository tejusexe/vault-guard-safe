...
const Accounts: React.FC = () => {
  ...
  const categories = data?.accounts.categories ?? [];
  ...
  const addCategory = async () => {
    ...
    await saveData(prev => ({
      ...prev,
      accounts: { categories: [...prev.accounts.categories, { id: uuid(), name, items: [] }] }
    }));
    setNewName("");
  };
  ...
  const deleteCategory = async (id: string) => {
    await saveData(prev => ({
      ...prev,
      accounts: { categories: prev.accounts.categories.filter(c => c.id !== id) }
    }));
  };
            ...
            <CardHeader className="flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button asChild variant="outline" size="sm">
                  <Link to={`/accounts/${cat.id}`}>Open</Link>
                </Button>
